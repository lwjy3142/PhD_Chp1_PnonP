...
```{r}
#set directory to there the genomicSEM outputs were kept 
setwd('robustness_check/')
...
#list files of SCZ nonp
files <- list.files(pattern = "^SCZnonp")

#check one output to get the colnames for the merged dataset
load(files[1])
test <- result[[1]]
colnames <- colnames(test)
remove(test) #remove the test file before merging

# initialize an empty data frame
combined_data <- data.frame(matrix(ncol = 27, nrow = 0))

#give colnames to the empty data frame perparing for merging
